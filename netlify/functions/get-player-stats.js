...
const { PLAYER_GIST_ID } = process.env;
...
exports.handler = async (event) => {
    ...
    if (!PLAYER_GIST_ID || !GITHUB_USERNAME) {
    ...
    const GIST_URL = `https://gist.githubusercontent.com/${GITHUB_USERNAME}/${PLAYER_GIST_ID}/raw/player-stats.json`;
    ...
    try {
        ...
        if (response.status === 404) {
            return { 
                ...
                body: JSON.stringify({
                    players: [],
                    goalkeepers: [],
                    buildReport: ["Data file has not been generated yet."]
                }) 
            };
        }
        
        if (!response.ok) {
            throw new Error(`Failed to fetch player stats Gist: ${response.statusText}`);
        }
        ...
        console.error("Error in get-player-stats:", error);
        return {
            statusCode: 500,
            headers: headers,
